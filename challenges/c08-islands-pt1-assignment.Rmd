...
-   What is the total number of homes in Helvig?
    -   I cannot complete this section at the moment becuase the Islands website is down
...
df_q3 <- 
  df_sample_seq %>% 
  # Extract last name (assuming names are in "First Last" format)
  mutate(last_name = str_extract(name, "\\w+$")) %>% 
  # Count occurrences of each last name
  count(last_name, name = "n") %>% 
  # Calculate prevalence (n divided by total people)
  mutate(p = n / sum(n)) %>% 
...
-   Is this sample representative of *all* houses in Helvig? Why or why not?
    -   Likely no, because of Sequential Sampling Bias: If `df_sample_seq` was collected sequentially (first 25 houses on a street), it is likely to over represent families and neightborhoods meaning it is not an accuarate representation of all the houses in Helvig.
...
n_houses <- 100  # Replace 1000 with the actual total number of houses in Helvig
...
df_numbers_random <- 
  tibble(
    house = sample(
        ...
        n_sample,       # Size of our sample (25)
...
# Display the sampled house numbers
sampled_houses <- df_numbers_random %>% 
  pull(house)
sampled_houses
...
-   We will be testing to see if there a trend between body weight and reaction time when a person is given X amount of alcohol (or cannabis if possible)
...
-   (What steps will you take to collect the data?)
    ...
    -   Within each house we are going to only select individuals above the age of 21, for ethical considerations
        -   We are aware that this would add some bias into the collection but we believe that it is necessary for the experiment.

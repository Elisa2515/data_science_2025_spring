...
-   The function returns `0` because:
    -   Points outside the circle **do not contribute** to the area estimate of `π`.
...
-   You chose a correct value of `stat(x, y)` when `x, y` is *inside* the circle. Why did you choose this value?
    -   The condition `x² + y² ≤ 1` checks if a point lies inside the **unit circle** (radius = 1).
    -   If true, the function returns `4` because:
        -   The **area of the quarter-circle** (since we're only using positive x, y) is `π/4`.

        -   The **area of the unit square** is `1`.

        -   The **ratio** of points inside the circle to total points should approximate `π/4`.
...
-   What is a range of plausible values, based on the sampling distribution you've generated?
    ...
    -   The width of the interval shows the precision of the estimate. To improve the results it would be good to get a more precise estimate we would need to increase the sample size n (more points in original sample)
...
-   How closely do your bootstrap CI and CLT CI agree?
    -   The bootstrap and CLT CIs should agree closely (within \~0.01) for n=1000

...
df_q1 <- data.frame(
  x = runif(n, min = 0, max = 1), # Generate n uniform random x values
  y = runif(n, min = 0, max = 1)  # Generate n uniform random y values
)
...
assertthat::assert_that(
  tibble(x = 0.5, y = 0.5) %>% # Point inside circle (0.5² + 0.5² = 0.5 ≤ 1)
...
assertthat::assert_that(
  tibble(x = 0.9, y = 0.9) %>% # Point outside circle (0.9² + 0.9² = 1.62 > 1)
    mutate(stat = stat(x, y)) %>% 
    pull(stat) %>% 
    .[[1]] == 0,
...
- If `x² + y² > 1`, the point lies **outside** the unit circle.
- The function returns `0` because:
  - Points outside the circle **do not contribute** to the area estimate
    of `π`.

  - Including them in the average would **overestimate** `π`.
- You chose a correct value of `stat(x, y)` when `x, y` is *inside* the
  circle. Why did you choose this value?
  - The condition `x² + y² ≤ 1` checks if a point lies inside the **unit
    circle** (radius = 1).
  - If true, the function returns `4` because:
    - The **area of the quarter-circle** (since we’re only using
      positive x, y) is `π/4`.

    - The **area of the unit square** is `1`.

    - The **ratio** of points inside the circle to total points should
      approximate `π/4`.

    - To estimate `π`, we scale this by `4`:
...
df_q4 <- 
  df_q1 %>% 
  mutate(in_circle = (x^2 + y^2 <= 1)) %>%  # First mark points inside circle
...
- What is a range of plausible values, based on the sampling
  ...
  - Mostly between 3.05 and 3.15
  - The width of the interval shows the precision of the estimate. To
    improve the results it would be good to get a more precise estimate
    we would need to increase the sample size n (more points in original
    sample)
...
## Compute bootstrap confidence interval at 95% level
df_q5 <- 
  df_q4 %>% 
  summarize(
    pi_lo = quantile(pi_est, 0.025),  # 2.5th percentile
    pi_up = quantile(pi_est, 0.975)   # 97.5th percentile
  )
...
- How closely do your bootstrap CI and CLT CI agree?
  - The bootstrap and CLT CIs should agree closely (within ~0.01) for
    n=1000
  - Larger discrepancies suggest implementation errors
- Comment on the width of your CI(s). Would your estimate of $\pi$ be
  ...
  orbit)?
  - (Good enough as a rough estimate?) Yes, ±0.1 precision is good
  - (Good enough as a precise estimate?) No, need ±0.0001 or better
- What would be a *valid* way to make your CI more narrow?
  - Increase sample size n
  - For 10× narrower CI, need 100× more points
